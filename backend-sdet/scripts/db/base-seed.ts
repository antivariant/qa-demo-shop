...
async function run() {
    try {
        console.log(`!!! STARTING DESTRUCTIVE SEED on project: ${projectId} !!!\n`);
        await clearCollection(COLLECTION);
        await loadCollection(COLLECTION, 'data/sdet_user.json');
        console.log('\n--- SDET user seeding completed successfully ---');
    } catch (error) {
        console.error('SDET user seeding failed:', error);
        process.exit(1);
    }
}
